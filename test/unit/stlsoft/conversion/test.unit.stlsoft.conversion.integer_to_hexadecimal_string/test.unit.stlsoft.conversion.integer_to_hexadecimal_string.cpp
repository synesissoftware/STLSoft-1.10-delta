/* /////////////////////////////////////////////////////////////////////////
 * File:        test.unit.stlsoft.conversion.integer_to_hexadecimal_string.cpp
 *
 * Purpose:     Implementation file for the test.unit.stlsoft.conversion.integer_to_hexadecimal_string project.
 *
...
# define STLSOFT_I2S_H2S_GET_DIGIT_CHARACTER(C)     get_hexadecimal_digit_character<C>()
	...
	)
	{
		...
		ss_size_t	n;
		char*		s	=	const_cast<char*>(unsigned_integer_to_hexadecimal_string(buf, cchBuf, low, &n));
		...
		else
		{
			...
			char const	ch = *s;
			ss_size_t	n2;
			char const*	s2 = unsigned_integer_to_hexadecimal_string(buf, cchBuf - 8, high, &n2);
	...
	)
	{
		...
		return i2s_handle_signed_minimum_in_hexadecimal_(buf, cchBuf, signed_t(i));
	}
 ...
 */
	...
	{
		uint32_t const	high	=	i >> 32;
		uint32_t const	low		=	i;

		ss_size_t	n;
		char*		s	=	const_cast<char*>(integer_to_hexadecimal_string(buf, cchBuf, low, &n));
		...
		else
		{
			...
			char const	ch = *s;
			ss_size_t	n2;
			char const*	s2 = integer_to_hexadecimal_string(buf, cchBuf - n, high, &n2);
...
int main(int argc, char **argv)
{
	...
	if(XTESTS_START_RUNNER("test.unit.stlsoft.conversion.integer_to_hexadecimal_string", verbosity))
	{
#ifdef _DEBUG
...
{
	char		buf[18]; // Max : 22 digits + '-' + '\0'
	char const*	s;
	int			v = 0;
	size_t		n;

	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), v, &n);
...
static void test_1_2()
{
	...
	XTESTS_TEST_MULTIBYTE_STRING_EQUAL("7f", s);
}
...
static void test_1_3()
{
	...
	XTESTS_TEST_POINTER_NOT_EQUAL(NULL, s);
	XTESTS_TEST_INTEGER_EQUAL(2u, n);
	XTESTS_TEST_MULTIBYTE_STRING_EQUAL("3f", s);
}
...
static void test_1_4()
{
	...
	s = stlsoft::integer_to_hexadecimal_string(buf, v, &n);
...
static void test_1_10()
{
	...
	{ for(size_t i = 0; i != STLSOFT_NUM_ELEMENTS(items); ++i)
	{
		char		buf[18]; // Max : 22 digits + '-' + '\0'
		size_t		n;
		char const*	s = stlsoft::integer_to_hexadecimal_string(buf, STLSOFT_NUM_ELEMENTS(buf), items[i].value, &n);
...
static void test_1_15()
{
	...
	{
        {   (stlsoft::sint32_t)0x80000000, "-80000000"   },
		{	-100000000,		"-5f5e100"		},
		{	-1000000,		"-f4240"		    },
		{	-10000,			"-2710"				},
		{	-100,			"-64"			    },
		...
		{	64000,			"fa00"				},
		{	1000000000,		"3b9aca00"			},
        {   (stlsoft::sint32_t)0x7fffffff, "7fffffff", },
	};
...
static void test_1_17()
{
	...
	{
        {   (stlsoft::sint64_t)0x8000000000000000, "-8000000000000000", },
		{	-100000000,		"-5f5e100"		},
		{	-1000000,		"-f4240"		    },
		{	-10000,			"-2710"				},
		{	-100,			"-64"			    },
		...
		{	64000,			"fa00"				},
		{	1000000000,		"3b9aca00"			},
        {   (stlsoft::sint64_t)0x7fffffffffffffff, "7fffffffffffffff", },
	};
...
static void test_1_20()
{
	...
	//char const* const	fmt = stlsoft::integral_printf_format_traits<integer_t>::hexadecimal_format_a(false);
	...
	//	size_t			n1;
	//	char const*		s	=	stlsoft::integer_to_hexadecimal_string(&buf1[0], STLSOFT_NUM_ELEMENTS(buf1), v, &n1);
...
static void test_1_40()
{
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), stlsoft::rand<integer_t>(), &n);
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), integer_t(i), &n);
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), integer_t(i) + 1000000000, &n);
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), integer_t(0), &n);
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), stlsoft::limit_traits<integer_t>::minimum(), &n);
	...
	{ for(size_t WARMUPS = 2; 0 != WARMUPS; --WARMUPS)
	{
		...
		{ for(size_t i = 0; i != N; ++i)
		{
			char		buf[18];
			size_t		n;
			char const*	s = stlsoft::integer_to_hexadecimal_string(&buf[0], STLSOFT_NUM_ELEMENTS(buf), stlsoft::limit_traits<integer_t>::maximum(), &n);
